...
sensor = RainSensor(port="/dev/ttyAMA1", baudrate=115200)
value = [0, 0, 0]
c = 0
j = 0
...
while True:
    ...
    value[j]=(rain_count / total_checks) * 100
    j=+1
    if j==2:
       if value[0] > 5 and value[1] > 5 and value[2] > 5:
           with open("/home/pi/raingauge/src/shared_data.txt", "w") as file:
               file.write(str('1'))
       else:
           with open("/home/pi/raingauge/src/shared_data.txt", "w") as file:
               file.write(str('0'))
