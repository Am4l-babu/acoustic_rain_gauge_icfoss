...
/**
  ******************************************************************************
  * @file  ulp_LoRa_Solar_Charge_Controller_Monitor_V1.0.0.ino
  * @author CDOH Team
  * @brief  Code to collect data from a battery management system
            using ULP_LoRa board and transmit the data via LoRa Communication
  *@verbatim
  ==============================================================================
                        ##### How to use #####
  ==============================================================================
      ...
      (+) Set the device address in HEX format
          static const u4_t DEVADDR = 0x00000000 ; 
      (+) Set the network session key in HEX format
          static const PROGMEM u1_t NWKSKEY[16] = { 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00, 0x00 };

      (+) Set the app session key in HEX format
...
static const PROGMEM u1_t NWKSKEY[16] = { 0x36, 0x22, 0x51, 0xDB, 0x85, 0x13, 0xDF, 0x92, 0x91, 0xCE, 0x61, 0xA6, 0x3D, 0x7E, 0x95, 0xA7 };

// LoRaWAN AppSKey, application session key
// This should also be in big-endian (aka msb).

static const u1_t PROGMEM APPSKEY[16] = {  0xD0, 0xCF, 0xEA, 0xB4, 0x62, 0x6A, 0xA2, 0x7B, 0x0E, 0x05, 0xDD, 0x53, 0x3B, 0x22, 0xC5, 0xB5 };

// LoRaWAN end-device address (DevAddr)
// See http://thethingsnetwork.org/wiki/AddressSpace
// The library converts the address to network byte order as needed, so this should be in big-endian (aka msb) too.

static const u4_t DEVADDR = 0x013DBAB2 ; // <-- Change this address for every node!
...
void data()
{
  ...
  delay(100);
  int i = 0, flag = 0, incomingByte = 0;
  unsigned long current_serial_time;
  while (1)
  {
    current_serial_time = millis();
    if (previous_serial_time > current_serial_time)
    {
    ...
    else
    {
      if ((current_serial_time - previous_serial_time) >= 30000)
      {
        ...
        for (i = 0; i < 4; i++)
          mydata[i] = 255;
        previous_serial_time = current_serial_time;
        Serial.println("end of data function");
        mySerial1.end();
        delay(200);
        return;
      }
