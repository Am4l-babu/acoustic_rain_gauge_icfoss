...
{
  String Accumulation;
  String Event_Accumulation;
  String Total_Accumulation;
  String Rain_Intensity;
...
bool call() {
  ...
  if (response.startsWith(F("Acc"))) {
    ...
    delay(1000);
    if (previous_accumulation != eventAcc)
    {
      i = 0;
      mydata.Accumulation = String(acc);
      mydata.Event_Accumulation = String(eventAcc);
      mydata.Total_Accumulation = String(totalAcc);
      mydata.Rain_Intensity = String(rInt);
      mydata.Device_status = "ON";
      delay(1000);
    }
    else
    {
    ...
    previous_accumulation = eventAcc;
