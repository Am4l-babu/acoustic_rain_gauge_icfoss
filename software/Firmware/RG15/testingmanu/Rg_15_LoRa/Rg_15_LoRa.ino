...
//RG_15();
    ...
    LMIC_setTxData2(1, (unsigned char *)&send_data, sizeof(send_data), 0);
...
void RG_15() {
  ...
  if (response.startsWith("Acc")) {
    ...
    //    Serial.print("Acc.= ");
    //    Serial.println(atof(acc), 3);
    //    Serial.println(atof(eventAcc), 3);
    //    Serial.println(atof(totalAcc), 3);
    //    Serial.println(atof(rInt), 3);
    //    Serial.println();
    mydata.Accumulation = 100 * atof(acc), 3;
    Serial.print("Accumulation= ");
    Serial.println(mydata.Accumulation);
    mydata.Event_accumulation = 100 * atof(eventAcc), 3;
    Serial.print("Event_accumulation= ");
    Serial.println(mydata.Event_accumulation);
    mydata.Total_accumulation = 100 * atof(totalAcc), 3;
    Serial.print("Total_accumulation= ");
    Serial.println(mydata.Total_accumulation);
    mydata.IPH = 100 * atof(rInt), 3;
    Serial.print("IPH= ");
    Serial.println(mydata.IPH);
    send_data[0]= byte(mydata.Accumulation);
    send_data[1]= highByte(mydata.Event_accumulation);
    send_data[2]= lowByte(mydata.Event_accumulation);
    send_data[3]= highByte(mydata.Total_accumulation);
    send_data[4]= lowByte(mydata.Total_accumulation);
    send_data[5]= highByte(mydata.IPH);
    send_data[6]= lowByte(mydata.IPH);
  }
