...
void RG_15_data()
{
  ...
  if (unoSerial.available() > 0)
  {
    ...
    unoSerial.flush();
    const char* cstring = response.c_str();
    Serial.println(response);

    if (response.startsWith(F("Acc"))) {

      char accB[7], eventAccB[7], totalAccB[7], rIntB[7], unit[4];
      sscanf (cstring, "%*s %s %[^,] , %*s %s %*s %*s %s %*s %*s %s", &accB, &unit, &eventAccB, &totalAccB, &rIntB);

      Accumulation += uint8_t(100 * atof(accB));
      Event_accumulation = uint16_t(100 * atof(eventAccB));
      Total_accumulation = uint16_t(100 * atof(totalAccB));
      IPH = uint16_t(100 * atof(rIntB));
    }
    unoSerial.flush();
    delay(100);
    unoSerial.end();
